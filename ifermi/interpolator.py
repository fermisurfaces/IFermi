...
BolzTraP2. Developed by Alex Ganose (https://gist.github.com/utf/160118f74d8a58fc9abf9c1c3f52a384).
"""
...
class Interpolater(object):
    ...
    def interpolate_bands(self, interpolation_factor: float = 5,
        ...
        equivalences = sphere.get_equivalences(
            atoms=self._atoms, nkpt=self._kpoints.shape[0] * interpolation_factor,
            magmom=self._magmom)

        # get the interpolation mesh used by BoltzTraP2
        interpolation_mesh = 2 * np.max(
            np.abs(np.vstack(equivalences)), axis=0) + 1

        for spin in self._spins:
            energies = self._band_structure.bands[spin] * units.eV
            data = DFTData(self._kpoints, energies, self._lattice_matrix,
                           mommat=self._mommat)
