...
class FermiSurfacePlotter(MSONable):
    ...
    @requires(crystal_toolkit, "crystal_toolkit option requires crystal_toolkit to be installed.")
    def plot_crystal_toolkit(self,
                             spin: Optional[Spin] = None,
                             colors: Optional[Union[str, dict, list]] = None,
                             opacity: float = 1.0) -> "Scene":
        """
        ...
        surfaces = []
        for c, (verts, faces) in zip(colors, isosurfaces):
            positions = [vert for triangle in verts[faces].tolist() for vert in
                         triangle]
        ...
        # alternatively,
        # cylinders have finite width and are lighted, but no strong reason to choose one over the other
        # cylinders = Cylinders(positionPairs=self.reciprocal_space.lines.tolist(), radius=0.01, color="rgb(0,0,0)")
        scene_contents.append(lines)

        spheres = []
        for position, label in zip(self._symmetry_pts[0], self._symmetry_pts[1]):
            sphere = Spheres(positions=[list(position)], tooltip=label, radius=0.05,
                             color="rgb(0, 0, 0)")
